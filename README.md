...
- Hemmer, C. J., & Durstewitz, D. (2026). True zero-shot inference of dynamical systems preserving long-term statistics. *Advances in Neural Information Processing Systems*, 38, 13497–13540. — *Uses Takens-style temporal delay embedding as the context signal to lift 1D observations into a higher-dimensional state space diffeomorphic to the true attractor (Eq. 5 in the paper). Introduces a zero-shot DSR framework (DynaMix) that preserves invariant long-term statistics; generalizes to unseen dynamical systems without task-specific retraining. See also [awesome-forecasting-papers](https://github.com/efmodeling/awesome-forecasting-papers).*
